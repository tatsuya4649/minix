...
// the following structure must conform to the stackframe_s structure used by the kernel's
// context switching code.
// if the structure is different,floating point registers must be added.
//
#if (CHIP == INTEL)
struct sigregs{
#if _WORD_SIZE == 4
	...
	int   sr_cx;
	int   sr_retreg;
	int   sr_retadr;	// return address to the saved caller --- used in kernel
...
struct sigframe{		// stack frame created for the signal process
...
struct sigregs{
	...
	long sr_a5;
	long sr_a6;
	long sr_sp;		// known as a7
	long sr_pc;
	short sr_psw;
	short sr_dummy;		// make size a multiple of 4 for system.c
};
#else
...
struct sigcontext{
	...
	struct sigregs sc_regs;		// register setted to restore
};
...
// value of sc_flag.must match <minix/jmp_buf.h>.
#define SC_SIGCONTEXT	2	// non-zero when the context of the signal is included
#define SC_NOREGLOCALS	4	// non-zero when the register cant't be saved and restore
