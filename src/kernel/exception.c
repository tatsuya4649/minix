// this file contains a simple exception handler.
// exceptions in user processes are converted to signals.
// exceptions in the kernel,MM and FS cause a panic.
//
//
#include "kernel.h"
#include <signal.h>
...
unsigned vec_nr;
{
	// an exception or unexpected interrupt has occured.
	struct ex_s{
		char *msg;
		int signum;
		int minprocessor;
	...
	saved_proc= proc_ptr;			// save proc_ptr,because it may be changed
						// by debug statements.
	ep = &ex_data[vec_nr];

	if (vec_nr == 2){			// spurious NMI of some machines
