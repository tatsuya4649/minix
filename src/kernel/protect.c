...
// to initialize code and data segment descriptors,and to initialize global descriptors
...
struct tss_s{
	...
	reg_t ss0;				// stack segment to use during interrupt
	reg_t sp1;
	reg_t ss1;
	reg_t sp2;
	reg_t ss2;
...
FORWARD _PROTOTYPE(void int_gate,(unsigned vec_nr,phys_bytes base,unsigned dpl_type));
FORWARD _PROTOTYPE(void sdesc,(struct segdesc_s *segdp,phys_bytes base,phys_bytes size));
...
{
	// set up table for protected mode.
	...
	gate_table[] = {
		...
		copr_error,COPROC_ERR_VECTOR,INPR_PRIVILEGE,
	...
	// this is caller early so you can't use the table set by main()
	data_bytes = (phys_bytes) size[1] << CLICK_SHIFT;
	if (size[0] == 0)
		code_bytes = data_bytes;		// general I&D
	else
	...
	* (u32_t *) dtp->base = vir2phys(idt);
	...
	// build segment descriptor for tasks and interrupt handlers.
	...
	init_dataseg(&gdt[DS_286_INDEX],(phys_bytes) 0,(phys_bytes) MAX_286_SEG_SIZE,TASK_PRIVILEGE);
	init_dataseg(&gdt[ES_286_INDEX],(phys_bytes) 0,(phys_bytes) MAX_286_SEG_SIZE,TASK_PRIVILEGE);
	...
	// initialized whenever a process's map is initialized changed.
	for (rp=BEG_PROC_ADDR,ldt_selector=FIRST_LDT_INDEX * DESC_SIZE;
		...
		init_dataseg(&gdt[ldt_selector / DESC_SIZE],vir2phys(rp->p_ldt),
				(pyhs_bytes) sizeof rp->p_ldt,INTR_PRIVILEGE);
	...
	// the pointer is set up so that an interrupt automatically saves 
	// the current process's registers ip::cs:f:sp:ss in the correct slots in the process table.
	tss.ss0 = DS_SELECTOR;
	init_dataseg(&gdt[TSS_INDEX],vir2phys(&tss),(phys_bytes) sizeof tss,INTR_PRIVILEGE);
	...
	for(gtp = &gate_table[0] ; gtp<&gate_table[sizeof gate_table / sizeof gate_table[0]]; ++gtp){
		int_gate(gtp->vec_nr,(phys_bytes) (vir_bytes) gtp->gate,PRESENT | INT_GATE_TYPE | (gtp->privilege << DPL_SHIFT));
	}
	int_gate(SYS_VECTOR,(phys_bytes) (vir_bytes) p_s_call,PRESENT | (USER_PRIVILEGE << DP_SHILT) | INT_GATE_TYPE);
	int_gate(LEVEL0_VECTOR, (phys_bytes) (vir_bytes) level0_call,PRESENT | (TASK_PRIVILEGE << DPL_SHIFT) | INT_GATE_TYPE);
	int_gate(SYS386_VECTOR,(phys_bytes) (vir_bytes) s_call,PRESENT | (USER_PRIVILEGE << DPL_SHIFT) | INT_GATE_TYPE);
}
/*======================================================================================*
 * 					init_codeseg					*
 *======================================================================================*/
...
int privilege;
{
	// build edscriptor for a data segment
	sdesc(segdp,base,size);
	segdp->access = (privilege << DPL_SHIFT) | (PRESENT | SEGMENT | WRITABLE);
			// EXECUTABLE = 0,EXPAND_DOWN = 0,ACCESSED = 0
}
/*======================================================================================*
 * 					sdesc	 					*
 *======================================================================================*/
PRIVATE void sdesc(segdep,base,size)
register struct segdesc_s *segdp;
phys_bytes base;
phys_bytes size;
{
	// fill int the size fields (base,limit and granularity) of a descriptor
	segdp->base_low = base;
	...
	if(size > BYTE_GRAN_MAX){
		...
		segdp->granularity = GRANULAR | (size >> (PAGE_GRAN_SHIFT + GRANULARIRY_SHIFT));
	}else{
		segdp->limit_low = size;
		segdp->granulariry = size >> GRANULARIRY_SHIFT;
	}
	segdp->granulariry |= DEFAULT;			// means BIG for data seg
}
/*======================================================================================*
 * 					seg2phys	 				*
 *======================================================================================*/
...
struct proc *pp;
{
	...
	// change the I/O permission lebel bits in the psw.
	// these specify least-privileged current permission level allowed to execute I/O instructions.
	// users and servers have CPL 3.
