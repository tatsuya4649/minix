...
typedef struct tty{
	...
	u16_t	*tty_inhead;		// pointer to place where next chra goes
	...
	devfun_t	tty_devread;	// routine to rad from low level buffers
	devfun_t	tty_icancel;	// cancel any device input
	int	tty_min;		// minimum required chars numver in inpur queue
	...
	int	tty_incum;		// chars count input so far
	char	tty_outrepcode;		// reply code,TASK_REPLY or REVIVE
	char	tty_outcaller;		// process that made the call (usually FS_
	...
	char	tty_iocaller;		// process that made the call (usuall FS)
	...
	struct winsize	tty_winsize;	// window size(#lines and #colums)

	u16_t	tty_inbuf[TTY_IN_BYTES];	// tty input buffer
} tty_t;

EXTERN tty_t	tty_table[NR_CONS+NR_RS_LINES_NR_PTYS];

// value of field
#define NOT_ESCAPED	0		// previous character is not LNEXT(^V)
#define ESCAPED		1		// previous character was LNEXT (^V)
#define RUNNING		0		// not STOP(^S) has been typed to stop output
...
#define IN_EOF		0x2000		// char is EFO(^D),do not return to user
