...
#define BLANK_NEM	((u16_t *) 0)
#define CONS_RAM_WORDS	80
#define MAX_ESC_PARMS	2
...
register struct tty *tp;
{
	...
	do{
		...
		user_phys = proc_vir2phys(proc_addr(tp->tty_outproc),
				tp->tty_out_vir);
...
int c;
{
	...
	switch(c){
		...
		case '\b':
			...
			return;
		case '\n':
			if ((cons->c_tty->tty_termios.c_oflag &
					(OPOST | ONLCR)) == (OPOST | ONLCR)){
				cons->c_column = 0;
			}
		case 013:
		case 014:
		...
		default:
			...
			if (cons->c_rwords == buflen(cons->c_rawqueue))
				flush(cons);
			cons->c_rawqueue[cons->c_rwords++] = cons->c_attr |
				(c & BYTE);
			cons->c_column++;
			return;
	}
...
int dir;
{
	...
	flush(cons);
	chars = scr_size - scr_width;

	if (dir == SCROLL_UP){
		if (softscroll){
			vid_vid_copy(cons->c_start+scr_width,cons->c_start,chars);
		}else
		if (!wrap && cons->c_org + scr_size + scr_width >=
				cons->c_limit){
		...
		new_line = (cons->c_org + chars) & v_mask;
	} else{
		if (softscroll){
...
register console_t *cons;
{
	...
	tty_t *tp = cons->c_tty;
	if (cons->c_rwords > 0){
		mem_vid_copy(copns->c_ramqueue,cons->c_cur,cons->c_rwords);
		cons->c_rwords = 0;
	...
	if (cons->c_esc_intro == '['){
		...
		switch(c){
			...
			case 'M':
				...
				dst = cons->c_org + cons->c_row * cons->c_width;
				src = dst + n * scr_width;
				count = (scr_lines - cons->c_row - n) * scr_width;
				vid_vid_copy(src,dst,count);
				blank_color = cons->c_blank;
...
{
	lock();
	out_byte(vid_port + INDEX,reg);
	out_byte(vid_port + DATA,(val>>8) & BYTE);
	out_byte(vid_port + INDEX, reg+1);
	out_byte(vid_port + DATA, val&BYTE);
	unlock();
}
...
PRIVATE void beep()
{
	...
	if (beeping) return;
	out_byte(TIMER_MODE,0xB6);
	out_byte(TIMER2,BEEP_FREQ & BYTE);
	out_byte(TIMER2,(BEEP_FREQ >> 8) & BYTE);
	lock();
...
tty_t *tp;
{
	...
	phys_copy(0x463L,vie2phys(&bios_crtbase),2L);
	...
	init_dataseg(&gdt[VIDEO_INDEX],vid_base,(phys_bytes) vid_size,TASK_PRIVILEGE);
	vid_size >>= 1;
	vid_mask = vid_size - 1;
